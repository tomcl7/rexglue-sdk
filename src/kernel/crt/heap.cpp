...
bool ReXHeap::Init(uint32_t heap_size_bytes) {
  ...
  initial_segment_size_ = AlignUp(std::max(heap_size_bytes, kMinSegmentSize), O1HEAP_ALIGNMENT);
...
bool ReXHeap::AllocateSegmentLocked(uint32_t segment_size_bytes) {
  ...
  segment_size_bytes = AlignUp(std::max(segment_size_bytes, kMinSegmentSize), O1HEAP_ALIGNMENT);
...
uint32_t ReXHeap::AllocLocked(uint32_t size, bool zero) {
  ...
  const uint32_t min_required_segment = AlignUp(alloc_size + kHeaderSize, O1HEAP_ALIGNMENT);
