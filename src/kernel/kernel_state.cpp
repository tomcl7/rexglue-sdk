...
void KernelState::SetupInputDrivers() {
  // Initialize input system with SDL driver if available, NOP driver as fallback
  input_system_ = std::make_unique<rex::input::InputSystem>(nullptr);

  if (!emulator_->is_tool_mode()) {
    auto sdl_driver = std::make_unique<rex::input::sdl::SDLInputDriver>(nullptr, 0);
    if (sdl_driver->Setup() == X_STATUS_SUCCESS) {
      input_system_->AddDriver(std::move(sdl_driver));
      REXKRNL_INFO("KernelState: SDL input driver initialized");
    } else {
      REXKRNL_WARN("KernelState: SDL input driver failed to initialize");
    }
  }

  // NOP driver (primary in tool mode, fallback otherwise)
  uint8_t nop_index = emulator_->is_tool_mode() ? 0 : 1;
  input_system_->AddDriver(std::make_unique<rex::input::nop::NopInputDriver>(nullptr, nop_index));
  input_system_->Setup();

  if (emulator_->is_tool_mode()) {
    REXKRNL_INFO("KernelState: Input system initialized (tool mode - NOP only)");
  }
}
...
object_ref<XThread> KernelState::LaunchModule(object_ref<UserModule> module) {
  ...
  REXKRNL_INFO("KernelState: Launching module...");
  ...
  if (XFAILED(result)) {
    REXKRNL_ERROR("Could not create launch thread: {:08X}", result);
    return nullptr;
  }
...
void KernelState::SetExecutableModule(object_ref<UserModule> module) {
  ...
  auto export_entry = emulator_->processor()->export_resolver()->GetExportByOrdinal(
      "xboxkrnl.exe", xboxkrnl::ordinals::XexExecutableModuleHandle);
  if (export_entry) {
    assert_not_zero(export_entry->variable_ptr);
    auto variable_ptr = memory_->TranslateVirtual<be<uint32_t>*>(
  ...
  export_entry = emulator_->processor()->export_resolver()->GetExportByOrdinal(
      "xboxkrnl.exe", xboxkrnl::ordinals::ExLoadedImageName);
  if (export_entry) {
    char* variable_ptr =
        memory_->TranslateVirtual<char*>(export_entry->variable_ptr);
    rex::string::util_copy_truncating(
    ...
    dispatch_thread_ =
        object_ref<XHostThread>(new XHostThread(this, 128 * 1024, 0, [this]() {
          ...
          while (dispatch_thread_running_) {
            ...
            REXKRNL_DEBUG("Dispatch thread processing queued item ({} remaining)", dispatch_queue_.size());
            global_lock.unlock();

            fn();
            REXKRNL_DEBUG("Dispatch thread completed item");
          }
          return 0;
        }));
...
object_ref<UserModule> KernelState::LoadUserModule(
  ...
  if (module->is_dll_module() && module->entry_point() && call_entry) {
    ...
    REXKRNL_WARN("LoadUserModule: DllMain(DLL_PROCESS_ATTACH) not implemented");
  }
...
void KernelState::UnloadUserModule(const object_ref<UserModule>& module,
  ...
  if (module->is_dll_module() && module->entry_point() && call_entry) {
    ...
    REXKRNL_WARN("UnloadUserModule: DllMain(DLL_PROCESS_DETACH) not implemented");
  }
...
void KernelState::TerminateTitle() {
  REXKRNL_DEBUG("KernelState::TerminateTitle");
  auto global_lock = global_critical_region_.Acquire();
...
void KernelState::BroadcastNotification(XNotificationID id, uint32_t data) {
  REXKRNL_DEBUG("BroadcastNotification(id={:#x}, data={}) to {} listeners",
...
void KernelState::CompleteOverlappedDeferred(
    ...
    X_RESULT result, std::move_only_function<void()> pre_callback,
    std::move_only_function<void()> post_callback) {
  CompleteOverlappedDeferredEx(std::move(completion_callback), overlapped_ptr,
                               result, result, 0, std::move(pre_callback), std::move(post_callback));
}
...
void KernelState::CompleteOverlappedDeferredEx(
    ...
    std::move_only_function<void()> post_callback) {
  REXKRNL_DEBUG("CompleteOverlappedDeferredEx: queuing for overlapped {:08X}", overlapped_ptr);
  auto ptr = memory()->TranslateVirtual(overlapped_ptr);
  XOverlappedSetResult(ptr, X_ERROR_IO_PENDING);
  XOverlappedSetContext(ptr, XThread::GetCurrentThreadHandle());
  auto global_lock = global_critical_region_.Acquire();
  dispatch_queue_.push_back([this, overlapped_ptr,
                             completion_callback = std::move(completion_callback),
                             pre_callback = std::move(pre_callback),
                             post_callback = std::move(post_callback)]() mutable {
    REXKRNL_DEBUG("Deferred overlapped {:08X}: running pre_callback", overlapped_ptr);
    if (pre_callback) {
      pre_callback();
    }
    REXKRNL_DEBUG("Deferred overlapped {:08X}: sleeping {}ms", overlapped_ptr, kDeferredOverlappedDelayMillis);
    rex::thread::Sleep(
        std::chrono::milliseconds(kDeferredOverlappedDelayMillis));
    uint32_t extended_error, length;
    REXKRNL_DEBUG("Deferred overlapped {:08X}: running completion", overlapped_ptr);
    auto result = completion_callback(extended_error, length);
    REXKRNL_DEBUG("Deferred overlapped {:08X}: completing with result {:08X}", overlapped_ptr, result);
    CompleteOverlappedEx(overlapped_ptr, result, extended_error, length);
    if (post_callback) {
      REXKRNL_DEBUG("Deferred overlapped {:08X}: running post_callback", overlapped_ptr);
      post_callback();
    }
  });
  dispatch_cond_.notify_all();
...
bool KernelState::Save(stream::ByteStream* stream) {
  REXKRNL_DEBUG("Serializing the kernel...");
  ...
  REXKRNL_DEBUG("Serializing {} threads...", threads.size());
  for (auto thread : threads) {
    ...
    if (!thread->Save(stream)) {
      REXKRNL_DEBUG("Failed to save thread \"{}\"", thread->name());
      num_threads--;
    }
  }
  ...
  *num_threads_ptr = static_cast<uint32_t>(num_threads);
  ...
  REXKRNL_DEBUG("Serializing {} objects...", num_objects);
  for (auto object : objects) {
    ...
    if (!object->Save(stream)) {
      REXKRNL_DEBUG("Did not save object of type {}", object->type());
      assert_always();
...
bool KernelState::Restore(stream::ByteStream* stream) {
  ...
  REXKRNL_DEBUG("Loading {} threads...", num_threads);
  ...
  REXKRNL_DEBUG("Loading {} objects...", num_objects);
